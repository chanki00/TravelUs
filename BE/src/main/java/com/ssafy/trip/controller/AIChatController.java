...
public class AIChatController {
    ...
    @PostMapping("/recommend-images")
    public ResponseEntity<?> recommendImages(@RequestBody Map<String, Object> request) {
        log.info("이미지 추천 요청: {}", request);
        
        // 여행 계획 정보 추출
        String title = (String) request.get("title");
        String description = (String) request.get("description");
        List<Map<String, Object>> itinerary = (List<Map<String, Object>>) request.get("itinerary");
        
        // 프롬프트 구성
        StringBuilder prompt = new StringBuilder();
        prompt.append("다음 여행 계획에 적합한 대표 이미지 5개를 추천해주세요:\n\n");
        prompt.append("여행 제목: ").append(title).append("\n");
        prompt.append("여행 설명: ").append(description).append("\n\n");
        prompt.append("일정 정보:\n");
        
        // 일정 정보 추가
        for (Map<String, Object> day : itinerary) {
            int dayNumber = ((Integer) day.get("day")) + 1;
            prompt.append(dayNumber).append("일차: ");
            
            List<Map<String, Object>> items = (List<Map<String, Object>>) day.get("items");
            if (items != null && !items.isEmpty()) {
                for (int i = 0; i < items.size(); i++) {
                    Map<String, Object> item = items.get(i);
                    prompt.append(item.get("title"));
                    if (i < items.size() - 1) {
                        prompt.append(", ");
                    }
                }
        ...
        prompt.append("\n이 여행 계획을 대표할 수 있는 아름다운 이미지 5개를 추천해주세요. ");
        prompt.append("각 이미지에 대한 설명과 함께 이미지 URL을 제공해주세요. ");
        prompt.append("이미지는 실제 존재하는 URL을 제공해주세요. ");
        prompt.append("다음 JSON 형식으로 응답해주세요:\n\n");
        prompt.append("{\n");
        prompt.append("  \"images\": [\n");
        prompt.append("    {\n");
        prompt.append("      \"description\": \"이미지 설명\",\n");
