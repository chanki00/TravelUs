  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="flex flex-col md:flex-row gap-4">
          ...
          <div class="flex-1">
            ...
            <div class="space-y-2">
              ...
              <div v-if="place.phone" class="flex items-start gap-2">
                <phone-icon class="h-5 w-5 text-gray-500 mt-0.5" />
                <span>{{ place.phone }}</span>
              </div>
              
              <div v-if="place.website" class="flex items-start gap-2">
                <globe-icon class="h-5 w-5 text-gray-500 mt-0.5" />
                <a :href="place.website" target="_blank" class="text-blue-600 hover:underline">
                  웹사이트 방문
                </a>
              </div>
            </div>
      ...
      <div class="p-4 border-t flex justify-end">
        <button 
          @click="$emit('close')" 
          class="px-4 py-2 border rounded-md hover:bg-gray-50 mr-2"
        >
          닫기
        </button>
        <button 
